...
$query = $db->query("SELECT * FROM calisanlar WHERE id = '{$id}'")->fetch(PDO::FETCH_ASSOC);
                        
if(isset($_POST['btn_kaydet'])){
    $query = $db->prepare("UPDATE calisanlar SET
	resim=:resim,
   	isim=:isim,
    pozisyon=:pozisyon
...
if ( $update ){
    ...
    header("Location: a-calisanlar.php");
}
else{
...
elseif(isset($_POST['btn_sil'])){
            ...
            $query = $db->prepare("DELETE FROM calisanlar WHERE id = :id");
			 ...
			 <form id="form" class="" action="haberler.php" name="form" method="POST">
					...
					<div class="feedback col-md-10">
							...
							<div class="email-input">
								...
								<div class="midemail">
								<img src="images/feedback/screen.png" alt=""> Resim
								</div>
								...
								<input type="text" name="name" value="<?=$query['isim'] ?>" placeholder="İsim Giriniz"> 
							</div>
							...
							<div class="email-input">
								...
								<input type="text" name="name" value="<?=$query['pozisyon'] ?>" placeholder="Pozisyon Giriniz"> 
							</div>
