...
if ( $update ){
    ...
    header("Location: a-haberler.php");
}
else{
    echo'<script type="text/javascript"> alert("hatalı kayıt");</script> ';
}
}
elseif(isset($_POST['btn_haber_sil']))
{
...
elseif(isset($_POST['btn_haber_aktif_pasif']))
{
    $aktiflik=0;
    if($query['aktif_mi']==0){
        $aktiflik=1;
  ...
  <body id="page-top">
   ...
   <nav class="navbar navbar-expand-md navbar-light fixed-top sticky-top nav-menu">
    ...
    $('#summernote').summernote('code', '<?=$query['metin'] ?>');
							...
							<input name="btn_haber_sil" type="submit" value="Haberi Sil">
							<input name="btn_haber_aktif_pasif" type="submit" value="<?php if($query['aktif_mi']==1){?>Pasifleştir<?php }else{ ?>Aktifleştir<?php } ?>">
							</div>
